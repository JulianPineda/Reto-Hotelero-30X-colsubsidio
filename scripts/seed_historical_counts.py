...
por (bodega, artículo), turno "morning", con cantidades estables (±5% de
ruido) en las últimas 5 semanas — una serie "tranquila" que un Auditor NO
debería marcar, para que una anomalía real durante la demo resalte con
claridad contra ese fondo.
...
async def seed() -> None:
    async with AsyncSessionLocal() as session:
        warehouses = (
            (await session.execute(select(Warehouse).order_by(Warehouse.code).limit(WAREHOUSE_COUNT)))
            .scalars()
            .all()
        )
        items = (
            (
                await session.execute(
                    select(CatalogItem).order_by(CatalogItem.oracle_code).limit(ITEMS_PER_WAREHOUSE)
                )
            )
            .scalars()
            .all()
        ...
        print(
            ...
            f"({len(warehouses)} bodegas x {len(items)} artículos x {HISTORY_WEEKS} semanas)"
        )
